...
import { SmsProviderConfig, SmsDefaultTemplate, SmsHttpTemplate, SmsZxjcTemplate, SmsVolcengineTemplate, VerifyCodeResult } from "../../../clients/src/internal/sms";
import { MobileAuth } from '@prisma/client';
import { DoFeApp } from "../../../common/src/config/dto/config.dto";
/**
 * 支持的 SMS 供应商
 */
export type SmsVendor = 'aliyun' | 'tencent' | 'http' | 'zxjcsms' | 'volcengine';
/**
 * SMS 供应商显示名称映射
 */
export declare const SMS_VENDOR_NAMES: Record<SmsVendor, string>;
/**
 * 统一的 SMS 模板类型
 */
export type SmsTemplate = SmsDefaultTemplate | SmsZxjcTemplate | SmsHttpTemplate | SmsVolcengineTemplate;
/**
 * 模板元数据接口（所有模板的公共字段）
 */
...
export interface IVolcengineSmsClient extends ISmsClient {
     ...
     */
    sendVerifyCode(phoneNumber: string, template: SmsVolcengineTemplate): Promise<any>;
    /**
     * 校验验证码
     *
     * @param phoneNumber - 手机号码
     ...
     * @param template - 火山引擎模板配置
     */
    checkVerifyCode(phoneNumber: string, code: string, template: SmsVolcengineTemplate): Promise<VerifyCodeCheckResult>;
}
/**
 * 统一的短信发送结果
 */
 ...
 * SMS 客户端创建函数类型
 */
export type SmsClientCreator = (config: SmsProviderConfig, dependencies: SmsClientDependencies) => ISmsClient;
/**
 * SMS 客户端工厂配置
 */
 ...
 * 设备发送日志 Redis Key 前缀
 */
export declare const DEVICE_SEND_LOGGER_KEY = "mobileCodeDevice";
/**
 * 默认发送频率限制（秒）
 */
export declare const DEFAULT_SEND_FREQUENCY = 30;
/**
 * 默认验证码过期时间（秒）
 */
export declare const DEFAULT_CODE_EXPIRE = 300;
export { SmsProviderConfig, SmsDefaultTemplate, SmsHttpTemplate, SmsZxjcTemplate, SmsVolcengineTemplate, VerifyCodeResult, } from "../../../clients/src/internal/sms";
