...
import { JwksClient } from '@dofe/infra-jwt';
...
/** Singleton JwksClient instances keyed by JWKS URI */
...
export class AuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
      ...
      try {
        ...
        if (jwtConfig.oidcMode || jwtConfig.jwksUri) {
          ...
          if (alg === 'RS256') {
            ...
            const jwksClient = getJwksClient(jwtConfig.jwksUri);
    ...
    if (
      ...
      environmentUtil.isWeChatMiniProgram(request) &&
      isMpTest &&
