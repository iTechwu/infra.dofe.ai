/**
 * Feature Registry — 功能声明与按需校验
 *
 * 每个功能模块在此声明自己的配置需求（FeatureDescriptor），
 * 项目通过 .env 的 REQUIRED_FEATURES 或 yaml 的 app.requiredFeatures
 * 声明自己需要哪些功能。启动时仅校验已声明功能的配置完整性。
 */
import { z } from 'zod';
...
function isFeatureSatisfied(
  ...
  if (descriptor.requiredEnvVars?.length && descriptor.requiredKeysPaths?.length) {
    const envOk = descriptor.requiredEnvVars.every(
      (v) => ctx.env[v] !== undefined && ctx.env[v] !== '',
    );
    const keysOk =
      !!ctx.keys &&
      descriptor.requiredKeysPaths.every(
        (p) => getByPath(ctx.keys!, p) !== undefined,
      );
    if (!envOk && !keysOk) {
      missing.push(
