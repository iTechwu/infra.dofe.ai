...
const string_util_1 = __importDefault(require("./string.util"));
const timer_util_1 = __importDefault(require("./timer.util"));
const enviroment_util_1 = __importDefault(require("./enviroment.util"));
...
exports.default = {
     ...
     */
    isImageFile(ext) {
        return imageExtensions.some((extension) => ext.toLowerCase().endsWith(extension));
    },
    isVideoFile(ext) {
        return videoExtensions.some((extension) => ext.toLowerCase().endsWith(extension));
    },
    isAudioFile(ext) {
        return audioExtensions.some((extension) => ext.toLowerCase().endsWith(extension));
    },
    isPdfFile(ext) {
        return pdfExtensions.some((extension) => ext.toLowerCase().endsWith(extension));
    },
    isEbookFile(ext) {
        return ebookExtensions.some((extension) => ext.toLowerCase().endsWith(extension));
    },
    isDocumentFile(ext) {
        return documentExtensions.some((extension) => ext.toLowerCase().endsWith(extension));
    },
    buildPythonS3Uri(vendor, bucket, key, region = 'cn-sh2') {
        if (vendor === client_1.FileBucketVendor.oss) {
     ...
     * 作为底层构建函数，供转码等模块复用。
     */
    buildStorageUri(vendor, bucket, key) {
     ...
     */
    buildAudioKeyFromVideoKey(originalKey, format = 'mp3') {
    ...
    getPrivateBucketFromKey(key) {
        const arr = key.split('/');
        return { vendor: arr[0], bucket: arr[1], key };
    },
    ...
    addFileNameSuffix(fileName, suffix) {
    ...
    renameFileWithNumber(fileName, num, hasTime, unique) {
        ...
        unique = unique || false;
        if (unique) {
            baseName += '-' + string_util_1.default.stringGen(4);
        }
        return `${baseName}-${hasTime ? timer_util_1.default.getCurrentDateTimeFormatted() + (num > 0 ? '-' : 0) : ''}${num > 0 ? 'v' + num : ''}.${extension}`; // 组合新的文件名
    },
    renameFolderWithNumber(folderName, num, hasTime = false, formatDay = true) {
        hasTime = hasTime || false;
        return `${folderName}-${hasTime ? timer_util_1.default.getCurrentDateTimeFormatted(formatDay) + '-' : ''}v${num}`; // 组合新的文件名
    },
    /**
     * 获取文件扩展名
     *
    ...
    completeKeyString(fileKey) {
        const key = fileKey.key;
        const ext = fileKey?.ext ? `.${fileKey.ext}` : '';
        const nodeEnv = fileKey?.env || enviroment_util_1.default.getEnv();
        if (string_util_1.default.isUUID(key) && !string_util_1.default.isUUID(fileKey.key)) {
            return `${fileKey.bucket}/${nodeEnv}/files/${fileKey.key}`;
        }
        else if (string_util_1.default.isUUID(key)) {
            return `${fileKey.bucket}/${nodeEnv}/files/${fileKey.key}${ext}`;
        }
        return fileKey.key;
    },
    getS3Uri(bucket, key, region = 'cn-sh2') {
        return `s3://ucloud@${region}/${bucket}/${key}`;
    },
    transforS3UriToKey(s3Uri) {
        if (!s3Uri)
            return { bucket: '', key: '' };
