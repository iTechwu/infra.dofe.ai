...
export class AgentXFileClient implements OnModuleInit {
  ...
  async uploadFileFromPath(filePath: string): Promise<string> {
    this.logger.info(`Uploading file from path: ${filePath}`);
    ...
    if (filePath.startsWith('http://') || filePath.startsWith('https://')) {
      this.logger.info('Detected URL, downloading file...');
      ...
      try {
        ...
        this.logger.info(`File downloaded successfully: ${filename}`);
      } catch (error) {
      ...
      try {
        ...
        this.logger.info(`File read successfully: ${filename}`);
      } catch (error) {
    ...
    this.logger.info(`File uploaded successfully: ${result.url}`);
    return result.url;
  }
  ...
  async uploadFile(request: FileUploadRequest): Promise<FileUploadResponse> {
    this.logger.info(`Uploading file: ${request.filename}`);
    ...
    try {
      ...
      this.logger.info(`File uploaded successfully: ${validatedResponse.url}`);
