...
const enviroment_util_2 = __importDefault(require("../../../../utils/dist/enviroment.util"));
/**
...
class FileS3Client {
    ...
    // 优化配置标志
    enableRetryMechanism = process.env.S3_ENABLE_RETRY === 'true' || true; // 默认启用
    enableEnhancedLogging = process.env.S3_ENHANCED_LOGGING === 'true' || true; // 默认启用
    maxRetries = parseInt(process.env.S3_MAX_RETRIES) || 3;
    baseRetryDelay = parseInt(process.env.S3_RETRY_DELAY) || 1000;
    /**
    ...
    constructor(config, storageConfig, appConfig, redis, httpService, logger) {
        ...
        if (this.enableRetryMechanism || this.enableEnhancedLogging) {
            if (enviroment_util_2.default.isProduction()) {
     ...
     */
    async withRetry(operation, operationName, context, customMaxRetries) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (attempt < maxRetries) {
                    const delay = Math.min(this.baseRetryDelay * Math.pow(2, attempt - 1), 5000); // 指数退避，最大5秒
                    if (this.enableEnhancedLogging) {
                        this.logger.warn(`${operationName} attempt ${attempt} failed, retrying`, {
                            operation: operationName,
                            error: error?.message,
                            errorCode,
                            attempt,
     ...
     * 增强的错误处理 - 统一错误日志格式和分类
     */
    logOperationResult(operationName, success, context, error, duration) {
        if (!this.enableEnhancedLogging) {
            return;
        }
        const logData = {
    ...
    setClient() {
        try {
            ...
            // 创建内部客户端（生产环境使用内部端点）
            if (enviroment_util_1.default.isProduction() && this.config.internalEndpoint) {
            ...
            throw (0, api_exception_1.apiError)(errors_1.CommonErrorCode.S3ClientInitializationError);
        }
    }
    /**
     * 获取存储桶的字符串表示
     ...
     */
    async getMultipartUploadId(key, bucket) {
        ...
        if (!multipartUpload?.UploadId) {
            throw (0, api_exception_1.apiError)(errors_1.CommonErrorCode.InitiateMultipartUploadError);
        }
    ...
    async completeMultipartUpload(uploadId, key, parts, bucket) {
        ...
        try {
            await this.externalClient.send(new client_s3_1.CompleteMultipartUploadCommand(params));
        }
        catch (e) {
    ...
    async fileDownloader(source) {
        ...
        return this.withRetry(async () => {
            ...
            const data = await this.internalClient.send(new client_s3_1.GetObjectCommand(params));
            // 使用 transformToByteArray 转换流数据
            const buffer = Buffer.from(await data.Body.transformToByteArray());
            const duration = Date.now() - startTime;
            this.logOperationResult('fileDownloader', true, {
                ...context,
                size: buffer.length,
            }, null, duration);
            return buffer;
    ...
    async fileUploader(buffer, destination) {
        ...
        return this.withRetry(async () => {
            ...
            await this.internalClient.send(new client_s3_1.PutObjectCommand(params));
            const duration = Date.now() - startTime;
            this.logOperationResult('fileUploader', true, context, null, duration);
        }, 'fileUploader', context);
    }
    async fileDataUploader(base64Data, key, bucket) {
        const finalBucket = this.getBucketString(bucket);
        // 将base64数据转换为Buffer
     ...
     */
    async setFileContentDisposition(fileKey, bucket, maxRetries = 2) {
        const finalBucket = this.getBucketString(bucket);
        const startTime = Date.now();
        // 带重试机制的执行函数
        const executeWithRetry = async (attempt = 1) => {
            try {
                ...
                const copySource = encodeURIComponent(`${finalBucket}/${fileKey}`).replace(/%2F/g, '/');
                const params = {
                    Bucket: finalBucket,
                    Key: fileKey,
                ...
                await this.internalClient.send(new client_s3_1.CopyObjectCommand(params));
                const duration = Date.now() - startTime;
                ...
                if (nonRetryableErrors.includes(errorCode)) {
                    if (errorCode === 'NoSuchKey') {
                        this.logger.warn('File not found when setting Content-Disposition, skipping operation', {
                            fileKey,
                            bucket: finalBucket,
                            error: error.message,
                            attempt,
                        ...
                        this.logger.error('Non-retryable error setting Content-Disposition', {
                            fileKey,
                            bucket: finalBucket,
                            error: error.message,
                            errorCode,
                ...
                if (attempt < maxRetries) {
                    ...
                    this.logger.warn(`Setting Content-Disposition failed on attempt ${attempt}, retrying in ${delay}ms`, {
                        fileKey,
                        bucket: finalBucket,
                        error: error.message,
                        errorCode,
                    ...
                    this.logger.error('All retries exhausted for setting Content-Disposition', {
                        fileKey,
                        bucket: finalBucket,
                        error: error.message,
                        errorCode,
     ...
     */
    async getInternalSignedUrl(bucket, key) {
        try {
            const cmd = new client_s3_1.GetObjectCommand({
                Bucket: bucket,
                Key: key,
            });
            return await (0, s3_request_presigner_1.getSignedUrl)(this.internalClient, cmd, {
                expiresIn: 3600,
            });
        }
        catch (e) {
     ...
     */
    async signGetUrl(bucket, key, options) {
        try {
            const cmd = new client_s3_1.GetObjectCommand({
                Bucket: bucket,
                Key: key,
            });
            const url = await (0, s3_request_presigner_1.getSignedUrl)(this.externalClient, cmd, options);
    ...
    async getPresignedUrl(bucket, uploadId, key, partNumber) {
        ...
        try {
            ...
            const presignedUrl = await (0, s3_request_presigner_1.getSignedUrl)(this.externalClient, command, {
                expiresIn: 3600,
            });
    ...
    async uploadToken(bucket, options) {
        const finalBucket = this.getBucketString(bucket);
        try {
            ...
            const url = await (0, s3_request_presigner_1.getSignedUrl)(this.externalClient, command, {
    ...
    async listBuckets() {
        try {
            const cmd = new client_s3_1.ListBucketsCommand({});
    ...
    async deleteFile(fileKey, bucket) {
        ...
        const startTime = Date.now();
        // 验证 fileKey 不是 URL
        if ((0, class_validator_1.isURL)(fileKey)) {
            this.logOperationResult('deleteFile', false, context, new Error('Cannot delete URL, expecting file key'));
            return false;
        }
        try {
            return await this.withRetry(async () => {
                ...
                const duration = Date.now() - startTime;
                this.logOperationResult('deleteFile', true, {
                    ...context,
                    bucket: finalBucket,
                }, null, duration);
                return true;
    ...
    // OSS 文件复制
    async copyFile(sourceBucket, sourceObject, targetBucket, targetObject) {
        const copyParams = {
            CopySource: `${sourceBucket}/${sourceObject}`,
            Bucket: targetBucket,
            Key: targetObject,
        };
        const command = new client_s3_1.CopyObjectCommand(copyParams);
        const data = await this.internalClient.send(command);
        await (0, client_s3_1.waitUntilObjectExists)({ client: this.internalClient, maxWaitTime: 30000 }, { Bucket: targetBucket, Key: targetObject });
        return data;
    }
    replaceUrlToOwnDoamin(url) {
...
exports.FileS3Client = FileS3Client;
//# sourceMappingURL=file-s3.client.js.map