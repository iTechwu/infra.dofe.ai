...
const config_constants_1 = require("../../../../common/src/config/constant/config.constants");
 ...
 * - 不包含业务逻辑
 */
class FileTosClient extends file_s3_client_1.FileS3Client {
    ...
    setTosClient() {
        try {
            ...
            const extractEndpoint = (endpoint) => {
                if (!endpoint)
                    return '';
            ...
            if (enviroment_util_1.default.isProduction()) {
                this.tosInternalClient = new tos_sdk_1.TosClient({
                    accessKeyId: this.storageConfig.accessKey,
                    accessKeySecret: this.storageConfig.secretKey,
                    region: this.config.region,
     ...
     */
    async getVideoInfo(fileKey, internal = false, bucket) {
        ...
        try {
            ...
            if (result.data && result.data.content) {
                ...
                const dar = videoStream?.display_aspect_ratio ?? videoStream?.dar ?? null;
     ...
     */
    async getImageInfo(fileKey, internal = false, bucket) {
        const finalBucket = bucket || this.getBucketString();
        const tosClient = internal ? this.tosInternalClient : this.tosClient;
        try {
            ...
            if (result.data && result.data.content) {
                ...
                // 尝试解析 JSON 以验证格式
                try {
                    const rawImageInfo = JSON.parse(imageInfoJson);
                    // 提取宽高值（TOS 返回的是字符串格式）
     ...
     */
    async getAudioInfo(fileKey, internal = false, bucket) {
        const finalBucket = bucket || this.getBucketString();
        const tosClient = internal ? this.tosInternalClient : this.tosClient;
        try {
     ...
     * @returns 返回保存的截图文件键名
     */
    async getSnapshot(fileKey, internal = false, bucket, time = 0, width = 0, height = 0, format = 'jpg') {
        const finalBucket = bucket || this.getBucketString();
        const tosClient = internal ? this.tosInternalClient : this.tosClient;
        try {
            ...
            const snapshotFileName = `${nameWithoutExt}${config_constants_1.TRANSCODE_CONSTANTS.SUFFIXES.SNAPSHOT}${config_constants_1.TRANSCODE_CONSTANTS.EXTENSIONS.JPG}`;
            ...
            if (result.data) {
                ...
                throw new Error('Failed to get snapshot result from TOS response: missing data');
            }
        }
        catch (error) {
     ...
     */
    async getPrivateDownloadUrl(fileKey, expire = 3600, internal = false, bucket) {
        // 如果是公开存储桶，直接返回公开 URL
        ...
        try {
            ...
            await this.redis.saveData(this.urlRedisKey, cacheKey, signedUrl, cacheExpire);
    ...
    async getPrivateDownloadUrlWithoutCdnEncrypt(fileKey, expiresIn = 30, internal = false, bucket) {
        ...
        try {
            ...
            this.logger.info('Generated presigned URL for TOS using TOS SDK (without CDN encrypt)', {
                fileKey,
                bucket: finalBucket,
                expiresIn,
                internal,
...
exports.FileTosClient = FileTosClient;
//# sourceMappingURL=file-tos.client.js.map