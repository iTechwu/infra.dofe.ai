...
exports.VolcengineOpenspeechProvider = void 0;
 ...
 * ```
 */
class VolcengineOpenspeechProvider extends base_provider_1.BaseOpenspeechProvider {
    httpService;
    config;
    /**
     * 云服务商标识：火山引擎 TOS
     ...
     */
    constructor(logger, httpService, config) {
        super(logger);
    ...
    async submitTask(params) {
        ...
        if (!this.config.appId || !this.config.appAccessToken) {
            throw new Error('Volcengine OpenSpeech config missing appId or appAccessToken');
        }
        if (!this.config.uid) {
        ...
        // 构建回调 URL
        const { api: baseUrl } = enviroment_util_1.default.generateEnvironmentUrls();
        ...
        try {
            ...
            // 发送请求
            const submitResp = await (0, rxjs_1.firstValueFrom)(this.httpService.post(submitUrl, submitBody, { headers }));
            // 检查响应状态
     ...
     * ```
     */
    async queryTaskStatus(vendorTaskId) {
        if (!this.config) {
            throw new Error('Volcengine OpenSpeech config (tos) not found');
        }
        // 校验必要的配置项
        ...
        try {
            // 发送查询请求
            const queryResp = await (0, rxjs_1.firstValueFrom)(this.httpService.post(queryUrl, {}, { headers }));
            // 获取状态码和消息
            const statusCode = queryResp.headers['x-api-status-code'] || queryResp.data?.status_code;
            const message = queryResp.headers['x-api-message'] || queryResp.data?.message;
            // 处理成功状态
            if (statusCode === VolcengineStatusCode.SUCCESS) {
                const text = queryResp.data?.result?.text || queryResp.data?.result?.[0]?.text;
            ...
            // 处理处理中状态
            if (statusCode === VolcengineStatusCode.PROCESSING ||
...
exports.VolcengineOpenspeechProvider = VolcengineOpenspeechProvider;
//# sourceMappingURL=volcengine.provider.js.map