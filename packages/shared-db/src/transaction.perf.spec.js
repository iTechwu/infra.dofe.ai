...
describe('Transactional Decorator Overhead', () => {
    const TX_CLIENT = Symbol('TX_CLIENT');
    // 模拟装饰器包装函数
    function simulateTransactionalWrapper(context, originalMethod, mockTx) {
        return async () => {
            // 检查是否已在事务中
    ...
    it('should have minimal wrapper overhead', async () => {
        ...
        const wrappedMethod = simulateTransactionalWrapper(context, originalMethod, mockTx);
        for (let i = 0; i < iterations; i++) {
            const start = performance.now();
            await wrappedMethod();
            times.push(performance.now() - start);
...
//# sourceMappingURL=transaction.perf.spec.js.map