...
exports.default = {
    ...
    getEnv() {
        if (process.env?.NODE_ENV === 'prod' ||
            ...
            return client_1.FileEnvType.prod;
        }
        if (process.env?.NODE_ENV === 'produs') {
            return client_1.FileEnvType.produs;
        }
        if (process.env?.NODE_ENV === 'prodap') {
            return client_1.FileEnvType.prodap;
        }
        if (process.env?.NODE_ENV === 'test') {
            return client_1.FileEnvType.test;
        }
        return client_1.FileEnvType.dev;
    },
    isDevDebug() {
    ...
    generateEnvironmentUrls() {
        ...
        let domain, subDomain = '', apiSubDomain = '';
        switch (true) {
            case this.isDev():
                domain = 'dofe.ai';
                subDomain = 'www.local';
...
//# sourceMappingURL=enviroment.util.js.map